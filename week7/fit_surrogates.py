...
class surrogateFitting:
    ...
    def get_initial_sample(self, output):
        ...
        if self.initial_sample_type == "Hammersley":
            self.training_samples[output] = HammersleySampling(
                df, self.n_init, sampling_type="selection"
            ).sample_points()

        elif self.initial_sample_type == "LatinHypercube":
            self.training_samples[output] = LatinHypercubeSampling(
                df, self.n_init, sampling_type="selection"
            ).sample_points()
    ...
    def fit_surrogate(self, output):
        ...
        rbf_surr = PysmoSurrogate(
            rbf_train, self.input_labels, [output], self.input_bounds
        )
    ...
    def get_surrogate_metrics(self, output):
        ...
        # Predicted values
        self.data_pred = self.surrogates[output]["surrogate"].evaluate_surrogate(
            self.data
        )

        # Get misclassification rates
        if (
            output == "repassivation_corrosion_potential_difference"
            or "synthetic_potential_difference_V"
        ):
            label = output + "_True"
            self.data_pred[label] = self.data_pred[output] >= 0
            self.surrogates[output]["true_positive"] += [
                ((self.data[label] == True) & (self.data_pred[label] == True)).sum()
            ]
            self.surrogates[output]["true_negative"] += [
                ((self.data[label] == False) & (self.data_pred[label] == False)).sum()
            ]
            self.surrogates[output]["false_positive"] += [
                ((self.data[label] == False) & (self.data_pred[label] == True)).sum()
            ]
            self.surrogates[output]["false_negative"] += [
                ((self.data[label] == True) & (self.data_pred[label] == False)).sum()
            ]
            misclassified = (
                self.surrogates[output]["false_negative"][self.n_iter[output]]
                + self.surrogates[output]["false_positive"][self.n_iter[output]]
            )
            self.surrogates[output]["misclassification"] += [
                misclassified / self.n_total * 100
            ]
        ...
        if (
            output == "repassivation_corrosion_potential_difference"
            or output == "synthetic_potential_difference_V"
        ):
            data_scaling_range = self.data[
                (self.data[output] > -0.1) & (self.data[output] < 0.1)
            ]
            self.index_max_error_scaling_range = data_scaling_range.nlargest(
                self.n_add, output + "_AE"
            ).index
            ...
            for i in range(self.n_add):
                try:
                    ...
                    self.index_max_error_misclassified += list(
                        self.index_max_error_scaling_range[: self.n_add - i]
                    )
                    break
        else:
    ...
    def add_training_sample(self, type="worst_misclassified", output=None):
        ...
        for i in range(self.n_add):
            ...
            self.training_samples[output].loc[
    ...
    def plot_errors(self, output, folder):
        ...
        for i, err in enumerate(errors):
            ax[i].plot(
                range(1, self.n_iter[output] + 1),
                self.surrogates[output][err],
                color="r",
                linewidth=2,
                label=error_name[i],
            )
    ...
    def save_errors(self, output, folder):
        ...
        out_file = (
            ...
            + "_"
            + str(self.n_add)
            + f"_{self.basis}_errors_vs_iteration.csv"
        )
        long_path = r"\\?\\" + os.path.abspath(str(out_file))
    ...
    def surrogate_validation_plots(self, output, folder, material, ph=7.5):
        ...
        df_temp_1_do_1 = self.data[
            (self.data["temperature_C"] == temp_1) & (self.data["do_mg_L"] == do_1)
        ]
        df_temp_1_do_2 = self.data[
            (self.data["temperature_C"] == temp_1) & (self.data["do_mg_L"] == do_2)
        ]
        df_temp_2_do_1 = self.data[
            (self.data["temperature_C"] == temp_2) & (self.data["do_mg_L"] == do_1)
        ]
        df_temp_2_do_2 = self.data[
            (self.data["temperature_C"] == temp_2) & (self.data["do_mg_L"] == do_2)
        ]
        ...
        # plot surrogate fit
        plt.plot(
            df_temp_1_do_1["salinity_kg_kg"] * 1000,
            df_temp_1_do_1["pred"],
            color=colormap[0],
            label=f"T = {temp_1:.0f} C, DO = {do_1:.1f} mg/L",
        )
        plt.plot(
            df_temp_2_do_1["salinity_kg_kg"] * 1000,
            df_temp_2_do_1["pred"],
            color=colormap[1],
            label=f"T = {temp_2:.0f} C, DO = {do_1:.1f} mg/L",
        )
        plt.plot(
            df_temp_1_do_2["salinity_kg_kg"] * 1000,
            df_temp_1_do_2["pred"],
            color=colormap[2],
            label=f"T = {temp_1:.0f} C, DO = {do_2:.1f} mg/L",
        )
        plt.plot(
            df_temp_2_do_2["salinity_kg_kg"] * 1000,
            df_temp_2_do_2["pred"],
            color=colormap[3],
        ...
        plt.xlim(
            min(self.data["salinity_kg_kg"]) * 1000,
            max(self.data["salinity_kg_kg"]) * 1000,
        )
        plt.tight_layout()
        out_file = (
            folder
            ...
            + "_"
            + str(self.n_add)
            + f"_{self.basis}_surrogate OLI data"
        )
...
def update_error_metrics(error_metrics, sf_object, output):
    ...
    error_metrics["false_positive"] += [
        sf_object.surrogates[output]["false_positive"][-1]
    ]
    error_metrics["false_negative"] += [
        sf_object.surrogates[output]["false_negative"][-1]
    ]
    error_metrics["true_positive"] += [
        sf_object.surrogates[output]["true_positive"][-1]
    ]
    error_metrics["true_negative"] += [
        sf_object.surrogates[output]["true_negative"][-1]
    ]
    error_metrics["misclassification"] += [
        sf_object.surrogates[output]["misclassification"][-1]
    ]
    ...
    error_metrics["balanced_accuracy"] += [
        sf_object.surrogates[output]["balanced_accuracy"][-1]
    ]
...
def fit_corrosion_surrogate(material, n_values, output, basis):
    ...
    for n in n_values:
        ...
        model = sf.surrogates[output]["surrogate"].save_to_file(
            folder
            + output
            + "_"
            + str(n[0])
            ...
            + str(n[3])
            + "_"
            + f"pysmo_rbf_{basis}_surrogate.json",
            overwrite=True,
        )
    ...
    error_df.to_csv(
        folder + output + f"_{basis}_surrogate_error_metrics.csv", index=False
    )
...
def fit_synthetic_surrogate(n, basis="cubic"):
    ...
    surrogate_file = (
        ...
        + "_"
        + str(n[3])
        + f"_pysmo_rbf_{basis}_surrogate.json"
    )
