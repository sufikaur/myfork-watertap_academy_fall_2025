...
def synthetic_parameter_sweep(T_range, do_range):
    ...
    for t in T_range:
        for d in do_range:
            ...
            results['synthetic_potential_difference_V'].append(get_synthetic_potential_difference(t, d))
    df = pd.DataFrame(results)
    df.to_csv('week7/synthetic_corrosion_data/synthetic_potential_difference.csv', index=False)
...
def plot_synthetic_data_vs_do(temps=[45, 95]):
    ...
    for i, t in enumerate(temps):
        ...
        plt.plot(data_t['do_mg_L'], data_t['synthetic_potential_difference_V'],marker='.',
             color=colors[i], markerfacecolor='white',markeredgecolor=colors[i], label=f'{t} C')
...
def plot_synthetic_data_vs_temp(do=[0, 1, 8]):
    ...
    for i, t in enumerate(do):
        ...
        plt.plot(data_t['temperature_C'], data_t['synthetic_potential_difference_V'],marker='.',
             color=colors[i], markerfacecolor='white',markeredgecolor=colors[i], label=f'{t} mg/L')
    plt.legend(frameon=False)
