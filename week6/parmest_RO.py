...
mS_to_massfrac = (
    0.67 * 1000
) / 1e6  # Conductivity to mass fraction for seawater based on SB data provided.
pressure_atmospheric = 101325
uS_to_massfrac = 0.67 / 1e6
flow_vol_scale = 1 / 1.909522782100071e-05  # 14318 # stdev of permeate flow #  1e2
mass_comp_scale = (
    1 / 2.4388414228794023e-06
)  # 379457 # stdev of permeate salinity #  1e4
...
def ro_parmest(data):
    ...
    if (
        m.fs.RO.config.concentration_polarization_type
    ...
    m.fs.RO.feed_side.properties_in[0].flow_mass_phase_comp["Liq", "H2O"].setub(
        1000
    )  # Bounds on flow rate too low!
    ...
    m.fs.feed.properties[0].flow_vol_phase.fix(
        gpm_to_m3ps * float(data["Qr+Qp+Qb_gpm"])
    )
...
def SSE(m, data):
    expr = 10 * flow_vol_scale * (
        (
            gpm_to_m3ps * float(data["Qp_gpm"])
            - m.fs.RO.mixed_permeate[0.0].flow_vol_phase["Liq"]
        )
        ** 2
    ) + 1000 * mass_comp_scale * (
        (
            1e-6 * float(data["Cp_ppm"])
...
def evaluate_runs(data, parameters, startrow=None, prefix=""):
    ...
    res_df = pd.DataFrame(
        columns=[
            ...
            "Permeate FR - actual",
            "Permeate FR - sim",
            "Permeate mass frac - actual",
            "Permeate mass frac - sim",
        ]
    )
    for i in range(startrow, startrow + data.shape[0]):
        test_data = {
            ...
            "Aw": parameters[0],
            "B": parameters[1],
            "Pf_psi": data["Pf_psi"][i],
            "Qr+Qp+Qb_gpm": data["Qr+Qp+Qb_gpm"][i],
            "RO_feed_mass_frac": data["RO_feed_mass_frac"][i],
            "RO_deltaP_psi": data["RO_deltaP_psi"][i],
            "RO_recovery_%": data["RO_recovery_%"][i],
        }
                    ...
                    [
                        {
                            ...
                            "Aw": parameters[0],
                            "B": parameters[1],
                            "Permeate FR - actual": gpm_to_m3ps * data["Qp_gpm"][i],
                            "Permeate FR - sim": value(
                                m.fs.RO.mixed_permeate[0.0].flow_vol_phase["Liq"]
                            ),
                            "Pct Error Permeate Flow": (
                                value(m.fs.RO.mixed_permeate[0.0].flow_vol_phase["Liq"])
                                - gpm_to_m3ps * data["Qp_gpm"][i]
                            )
                            / (gpm_to_m3ps * data["Qp_gpm"][i])
                            * 100,
                            "Permeate mass frac - actual": data["Cp_ppm"][i] * 1e-6,
                            "Permeate mass frac - sim": m.fs.RO.mixed_permeate[
                                0.0
                            ].mass_frac_phase_comp["Liq", "TDS"](),
                            "Pct Error Perm mass frac": (
                                value(
                                    m.fs.RO.mixed_permeate[0.0].mass_frac_phase_comp[
                                        "Liq", "TDS"
                                    ]
                                )
                                - data["Cp_ppm"][i] * 1e-6
        ...
        print(
            "'Pct Error Perm mass frac':",
            (
                value(m.fs.RO.mixed_permeate[0.0].mass_frac_phase_comp["Liq", "TDS"])
                - data["Cp_ppm"][i] * 1e-6
            )
            / (data["Cp_ppm"][i] * 1e-6)
            * 100,
    ...
    if not os.path.isfile(fname):
        with pd.ExcelWriter(fname) as writer:
            res_df.to_excel(
                writer, sheet_name="Sheet1", header=True, index=False, startrow=startrow
            )
