...
def get_chemistry_json_for_dbs(inflows, redox=None):
    ...
    dbs_data = {
        ...
        "params": {
            ...
            "phases": ["liquid1", "vapor", "solid", "liquid2"],
...
def process_condition(
    ...
    try:
        ...
        flash_input = {
            "params": {
                ...
                "inflows": {
                    ...
                    "totalAmount": {
                        "value": effective_H2O + original_salts,
                        "unit": "g",
                    },
                },
        ...
        flash_filename = os.path.join(
            output_folder, f"flash_output_temp_{T}_ph_{ph}_recovery_{rec}_do_{do}.json"
        )
        ...
        try:
            molecular_concentration = result_flash_calc["data"]["result"]["total"][
                "molecularConcentration"
            ]
        except Exception as e:
            print(
                f"Error extracting molecular concentration for T={T}, ph={ph}, rec={rec}, do={do}: {e}"
            )
            return None
        ...
        corrosion_spec["params"]["corrosionParameters"][
            "contactSurface"
        ] = CONTACT_SURFACE

        # Run the corrosion rate calculation.
        result_corrosion_rate_calc = oliapi.call(
            "corrosion-rates", chemistry_file_id, corrosion_spec
        )

        # Save the corrosion output.
        corrosion_filename = os.path.join(
            output_folder,
...
def run_engine_survey(
    config, oliapi, CONTACT_SURFACE="Duplex stainless 2205", cases=None
):
    ...
    if not chemistry_file_id:
        ...
        result = oliapi.generate_chemistry_file(
            "chemistry-builder", "", chemistry_payload
        )
        print(result)
    ...
    base_corrosion_spec = read_json_from_file(
        os.path.join("corrosion_example", "data", "specification"),
    ...
    if cases is None:
        ...
        total_tasks = (
            len(temperatures) * len(pH_values) * len(recovery_values) * len(do_values)
        )
