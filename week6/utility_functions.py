...
def load_data(full_data):
    ...
    dt['RO_feed_mass_frac'] = (1e-6 * (
                dt['RO Overall Concentrate EC (ppm)'] * dt['Qr_gpm'] + dt['CF Effluent EC (ppm)'] * dt['Qp+Qb_gpm'])
                               / dt['Qr+Qp+Qb_gpm'])
    ...
    subset_cols = ['flow_vol_in', 'mass_frac_TDS_in', 'pressure_in', 'deltaP', 'RO_recovery_%', 'flow_vol_permeate',
